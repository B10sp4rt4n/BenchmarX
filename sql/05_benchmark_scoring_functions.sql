-- ============================================
-- BenchmarX - Benchmark-Aware Scoring Functions
-- ============================================
-- Updates scoring functions to be benchmark-aware
-- Allows scoring per benchmark for comparison
-- ============================================

-- ============================================
-- FUNCTION: Calculate vendor score for a context AND benchmark
-- ============================================
...
DECLARE
    ...
    v_breakdown JSONB;
BEGIN
    -- Get benchmark_id (use active if not specified)
    IF p_benchmark_id IS NULL THEN
        v_benchmark_id := get_active_benchmark();
    ELSE
        v_benchmark_id := p_benchmark_id;
    END IF;
    ...
    -- Calculate weighted scores (NOW FILTERED BY BENCHMARK)
    WITH weighted_detections AS (
        ...
        WHERE dr.vendor_id = p_vendor_id
          AND dr.benchmark_id = v_benchmark_id  -- KEY CHANGE: Filter by benchmark
    ),
    score_calc AS (
        SELECT 
    ...
    SELECT 
        COALESCE(sc.total, 0),
        COALESCE(sc.max_possible, 0),
        CASE WHEN COALESCE(sc.max_possible, 0) > 0 
             THEN (COALESCE(sc.total, 0) / sc.max_possible * 100) 
             ELSE 0 
        END,
        COALESCE(sc.active_cnt, 0),
        COALESCE(sc.dynamic_cnt, 0),
        COALESCE(sc.no_evid_cnt, 0),
        COALESCE(sc.breakdown, '[]'::jsonb)
    INTO 
        v_total_score,
        v_max_score,
        score_percentage,
...
COMMENT ON FUNCTION calculate_vendor_score IS 'Core scoring function - calculates weighted score for vendor in specific context and benchmark';

-- ============================================
-- FUNCTION: Materialize scores for a specific benchmark
-- ============================================
...
BEGIN
    ...
    -- Delete existing scores for this benchmark/context/scoring_version combination
    DELETE FROM scored_results 
    WHERE benchmark_id = v_benchmark_id 
      AND scoring_version = v_scoring_version;
...
COMMENT ON FUNCTION materialize_scores_for_benchmark IS 'Batch function to calculate and store all vendor-context scores for a specific benchmark';

-- ============================================
-- FUNCTION: Get vendor ranking for a context and benchmark
-- ============================================
...
BEGIN
    ...
    WHERE sr.context_profile_id = p_context_profile_id
      AND sr.benchmark_id = v_benchmark_id
      AND sr.scoring_version = v_scoring_version
    ORDER BY sr.score_percentage DESC;
END;
$$ LANGUAGE plpgsql;

COMMENT ON FUNCTION get_vendor_ranking IS 'Returns ranked list of vendors for a specific context and benchmark';

-- ============================================
-- FUNCTION: Get category coverage for vendor and benchmark
-- ============================================
...
BEGIN
    ...
    SELECT 
        ...
        (SUM(CASE WHEN dr.detection_state IN ('ACTIVE', 'DYNAMIC') THEN 1 ELSE 0 END)::DECIMAL / 
         NULLIF(COUNT(a.id), 0) * 100) AS coverage_percentage
    ...
    LEFT JOIN detection_results dr ON a.id = dr.attack_id 
        ...
        AND dr.benchmark_id = v_benchmark_id
    GROUP BY ac.name
    ORDER BY coverage_percentage DESC NULLS LAST;
END;
$$ LANGUAGE plpgsql;

COMMENT ON FUNCTION get_category_coverage IS 'Returns detection coverage by attack category for a vendor in specific benchmark';

-- ============================================
-- FUNCTION: Compare vendor across benchmarks
-- ============================================
CREATE OR REPLACE FUNCTION compare_vendor_across_benchmarks(
    p_vendor_id UUID,
    p_context_profile_id UUID
)
RETURNS TABLE (
...
BEGIN
    ...
    WITH ranked_scores AS (
        SELECT 
            b.name AS bm_name,
            b.report_date AS bm_date,
            sr.score_percentage AS score_pct,
            RANK() OVER (PARTITION BY sr.benchmark_id ORDER BY sr.score_percentage DESC) AS bm_rank,
            LAG(sr.score_percentage) OVER (ORDER BY b.report_date) AS prev_score,
            LAG(RANK() OVER (PARTITION BY sr.benchmark_id ORDER BY sr.score_percentage DESC)) 
                OVER (ORDER BY b.report_date) AS prev_rank
        FROM scored_results sr
        JOIN benchmarks b ON sr.benchmark_id = b.id
        WHERE sr.vendor_id = p_vendor_id
          AND sr.context_profile_id = p_context_profile_id
          AND sr.scoring_version = (SELECT version FROM scoring_rules WHERE is_active = true)
    ...
    SELECT 
        ...
        score_pct,
        bm_rank::INTEGER,
        (score_pct - prev_score) AS score_change,
        (prev_rank - bm_rank)::INTEGER AS rank_change
    FROM ranked_scores
    ORDER BY bm_date DESC;
END;
$$ LANGUAGE plpgsql;

COMMENT ON FUNCTION compare_vendor_across_benchmarks IS 'Compare vendor performance across different benchmarks over time';
